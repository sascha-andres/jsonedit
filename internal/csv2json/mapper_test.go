...
func getStoreNewRecordFunc(test string) NewRecordFunc {
	...
	return func(record, header []string) {
		if _, ok := nrData[test]["expect"]; !ok {
			...
			nrData[test]["VALUATION_TIMESTAMP"] = time.Date(2021, 1, 1, 0, 0, 0, 0, time.UTC).Format("20060102150405")
			nrData[test]["PRODUCT_GROUP"] = "PG"
			nrData[test]["FILE_NAME"] = test
			nrData[test]["PROCESS"] = "PRC"
		}
	}
}
...
var afvFunctions = map[string]askForValueFuncFactory{
	"from-store": func(test string) AskForValueFunc {
		return func(record, header []string, field CalculatedField) (string, error) {
			if _, ok := nrData[test][field.Format]; !ok {
				return "", errors.New(fmt.Sprintf("no value set for %q", field.Format))
			}

			return nrData[test][field.Format], nil
		}
	},
}
