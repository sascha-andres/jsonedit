...
func (cs *Conditions) Apply(logger *slog.Logger, recordInfo map[string]any) bool {
	if cs == nil || len(*cs) == 0 {
		return false
	}
	for _, condition := range *cs {
		if !condition.Applies(logger, recordInfo) {
			return false
		}
	}
	return true
...
func (c *Condition) Applies(logger *slog.Logger, recordInfo map[string]any) bool {
	switch c.Type {
	case "int":
	...
	default:
		if logger != nil {
			logger.Warn("property configuration has a property defined but no (supported) type, condition check not possible", "key", c.Type)
		}
		return false
	}
}
...
func (c *Condition) intApplies(logger *slog.Logger, recordInfo map[string]any) bool {
...
func (c *Condition) floatApplies(logger *slog.Logger, recordInfo map[string]any) bool {
...
func (c *Condition) stringApplies(logger *slog.Logger, recordInfo map[string]any) bool {
...
func (c *Condition) boolApplies(logger *slog.Logger, recordInfo map[string]any) bool {
